...
def calculate_streak(entries):
    # Рассчитать текущую серию дней подряд
    if not entries:
        return 0
...
def get_smart_insights(entries):
    # Получить исследование состояния на основе записей
    if len(entries) < 3:
    ...
    # Лучший и худший день по настроению
...
def get_stats(telegram_id):
    # Получить статистику пользователя по его Telegram ID
    try:
        conn = get_db()
        ...
        # Получаем записи пользователя с тегами
        ...
        # Обрабатываем теги: преобразуем строку в список
        ...
        # Получаем исследование состояния
        ...
        return jsonify({
            ...
            'streak': streak,
            'insights': insights,
            'entries': entries[:10]  # Возвращаем только последние 10 записей
        })
...
def get_insights(telegram_id):
    # Получить только исследование состояния для пользователя
    try:
        conn = get_db()
...
def get_articles():
    # Получить все статьи из JSON файла
    try:
...
def get_videos():
    # Получить все видео из JSON файла
    try:
